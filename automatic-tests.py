...
rat_names = []
num_senders_list = [1, 2, 4, 8, 12, 16]
...
def run_test(cctype, rat_id, num_senders, cmd_line_args):
    global rat_names
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--input_rats', metavar='I', type=str, help='Directory where the dna files to be tested are stored')
    #argparser.add_argument('--final_output_folder', metavar='O', type=str, help='Directory where final statistics and graphs are to be written')
    argparser.add_argument('--raw_output_folder', metavar='R', type=str, help='Directory where raw results are to be written')
    argparser.add_argument('--server_ip', type=str, help="IP address od the server")
    argparser.add_argument('--source_interface', type=str, default='eth0', required=False, help='Interface from which to send. Defaults to eth0')
    ...
    while True:
        ...
        run_test(cctype, rat_id, num_senders, cmd_line_args)    