...
def plot_graphs(data_bins, num_senders_list, rat_names):
    ...
    for num_senders in  num_senders_list:
        ...
        for datapt in data_bins:
            ...
            rnd = [random.random() < 1.0 for x in data_bins[datapt]]
            plot_ellipse.plt.plot([data_bins[datapt][i][0] for i in range(len(data_bins[datapt])) if rnd[i]], [data_bins[datapt][i][1] for i in range(len(data_bins[datapt])) if rnd[i]], colors[datapt[1]]+'o', alpha=0.5)
